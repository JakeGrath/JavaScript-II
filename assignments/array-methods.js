...
names.first_name = names.first_name.toUpperCase();
return names;
});
console.log(allCaps); 
...
let ticketPriceTotal = runners.reduce(function(donations, val){
donations = donations + val["donation"];
return donations}, 0);
...
// Problem 1
//Create a list of companies that participated in the 5k run//
let companies = []
let companyDonations = runners.forEach(company => {
    if (companies.includes(company.company_name) === false){
...
// Problem 2
//Find a list of runners that donated >= $200, they receive sponsorship.//
let bigBallers = runners.filter(obj => {
    return obj.donation >= 200;
});
console.log(bigBallers);

// Problem 3
//Tally up shirt sizes for orders//
let sizeTally = {
    "XS" : 0,
    "S" : 0,
    "M" : 0,
...
let shirtSizes = runners.forEach(size => {
    return sizeTally[size.shirt_size] = sizeTally[size.shirt_size] + 1;
});

console.log(sizeTally);